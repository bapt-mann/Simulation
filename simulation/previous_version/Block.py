...
class Block:
    ...
    def __init__(self, _width, _height, _pos_x, _pos_y, _type):
        ...
        print(_type)
        print(Block.color_list[_type])
        self.color = Block.color_list[_type]

        self.collide = False

        # Attribution d'une image depuis le cache pygame.transform.smoothscale(Block.images["fire"], (self.size[0]+self.size[0]/5, self.size[1]+self.size[1]/5))
        if _type == 1:
            self.image = pygame.transform.smoothscale(Block.images["fire"], (self.size[0], self.size[1]))
            self.type = Type.fire
        elif _type == 4:
            self.image = pygame.transform.smoothscale(Block.images["water"], (self.size[0], self.size[1]))
            self.type = Type.water
        else:
            self.image = pygame.transform.smoothscale(Block.images["plant"], (self.size[0], self.size[1]))
    ...
    def collision_block(blockA, blockB):
        if blockB.right < blockA.left:
            # rectB est à gauche 
            return False

        if blockB.bottom < blockA.top:
            # rectB est au-dessus 
            return False

        if blockB.left > blockA.right:
            # rectB est à droite 
            return False

        if blockB.top > blockA.bottom:
    ...
    def set_state(self, type):
        ...
        self.type = Type[type]
        self.color = Block.color_list[self.type.value]
    ...
    def spawn_random_block( block_number, type, range_x=[0,100], range_y=[0,100], max_attempts=1000):
        ...
        for _ in range(block_number):
            ...
            if not placed:
                print(f"Impossible de placer un bloc après {max_attempts} essais.")
                break
